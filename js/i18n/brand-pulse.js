...
export default {
  ...
  "companion.log.manage": { en: "Manage", id: "Kelola" },
  ...
  "companion.memory.intro": { en: "Everything logged here — auto-detected signals, your own notes, and the Companion's replies — is what every AI feature reads as \"what's happening in this brand right now\". Delete anything you don't want remembered or read by AI going forward; this only stops future reads, it can't un-send something already sent in a past request. A deleted auto-signal can reappear later if the same condition is still true; a deleted note or reply never does.", id: "Semua yang tercatat di sini — sinyal otomatis, catatanmu sendiri, dan balasan Teman Brand — adalah yang dibaca semua fitur AI sebagai \"yang sedang terjadi di brand ini\". Hapus apa pun yang tidak mau diingat atau dibaca AI ke depannya; ini cuma menghentikan pembacaan ke depan, tidak bisa menarik kembali yang sudah terkirim di permintaan sebelumnya. Sinyal otomatis yang dihapus bisa muncul lagi kalau kondisinya masih berlaku; catatan atau balasan yang dihapus tidak akan muncul lagi." },
  "companion.memory.clearAll": { en: "Delete all history", id: "Hapus semua riwayat" },
  "companion.memory.clearConfirm.title": { en: "Delete all brand memory?", id: "Hapus semua memori brand?" },
  "companion.memory.clearConfirm.body": { en: "Every logged signal, note, and reply for this brand will be gone — AI features will start again with no recent history. This can't be undone.", id: "Semua sinyal, catatan, dan balasan yang tercatat untuk brand ini akan hilang — fitur AI mulai lagi tanpa riwayat terbaru. Tidak bisa dibatalkan." },
